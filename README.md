...
`npm install --save ngx-tree`

Now when you have `ngx-tree` installed, you are in a few steps from having tree in your application:
...
import { TreeModule } from 'ngx-tree';
...
2. As soon as the previous step is done we need to give ngx-tree a model to render - this can be accomplished by populating its `[tree]` attribute with an object that conforms to the `TreeModel` interface (see [API](#wrench-api)):

```typescript
// 1 - import required classes and interfaces
import { TreeModel } from 'ngx-tree';
...
3. Apart from that, in order to have usable tree in the browser, you need to add **ngx-tree styles** which you can find in your `node_modules/ngx-tree/styles.css`
...
```typescript
      "styles": [
        "styles.css",
        "../node_modules/ngx-tree/styles.css"
      ],
```

4. And finally, I suppose, you'd want to listen to events generated by ngx-tree (for a full list of supported events look at the [API](#wrench-api)). No problem, this is also easy to do - for example let's add a listener for `node was selected` kind of events:

```typescript
// 1 - import required classes and interfaces
import { TreeModel, NodeEvent } from 'ngx-tree';
...
Also in the next section, you'll be reading about events generated by the `ngx-tree`. And here [Tree](src/tree.ts) class comes in handy for us, because its instances propagated with event objects. Under the hood, `ngx-tree` wraps a `TreeModel` provided by the user in `Tree`. And `Tree` in turn has lots of useful methods and properties (like `parent`, `hasChild()`, `isRoot()` etc.)
...
System.config({
    ...
    map: {
      // ...
      'ngx-tree': 'node_modules/ngx-tree/bundles/ngx-tree.umd.min.js',
      // ...
    },
    // ...
}
